...
tape( 'the function returns a factory function (parameters)', function test( t ) {
	var createStream = factory( 20.0, 0.3 );
	t.strictEqual( typeof createStream, 'function', 'returns expected value' );
	t.end();
});
...
tape( 'the function returns a function which throws an error if `r` is not a positive number', function test( t ) {
	var values;
	var i;
...
tape( 'the function returns a function which throws an error if `r` is not a positive number (parameters)', function test( t ) {
	var values;
	var i;
...
tape( 'the function returns a function which throws an error if provided an options argument which is not an object (parameters)', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			var createStream = factory( 20.0, 0.3, value );
			createStream();
		};
	}
});
...
tape( 'the function returns a factory function which creates stream instances', function test( t ) {
	...
	for ( i = 0; i < 10; i++ ) {
		t.strictEqual( createStream( 20.0, 0.3 ) instanceof RandomStream, true, 'returns expected value' );
	}
	t.end();
});
